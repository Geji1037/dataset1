...
def main(
    ...
    if not lora_weights_path.endswith(".bin"):
        ...
        # —— 静态融合 global + local adapter —— 
        # 1) 先 load 全局 LoRA 权重
        global_state = torch.load(lora_weights_path, map_location=lambda s,l: s)
        # 2) 如果启用 local，则按 weight 比例合并 local_state
        if local:
            local_state = torch.load(local_model_path, map_location=lambda s,l: s)
            for k in global_state:
                if "lora_A" in k or "lora_B" in k:
                    global_state[k] = weight * global_state[k] + (1 - weight) * local_state[k]
        # 3) 一次性写入 default adapter

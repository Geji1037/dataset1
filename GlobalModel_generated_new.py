...
class EvalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # judge medical data or not 
        if "Question" in ques:
            instruction = ques["Question"]
        elif "instruction" in ques:
            instruction = ques["instruction"]
        else:
            raise ValueError(f"Sample lost 'Question' or 'instruction' ")
...
def main(
    ...
    lora_weights_path: str = "/root/.cache/modelscope/hub/models/LLM-Research/Fedray/FedDPA/code/lora-medical/12/9/adapter_model.bin",
    lora_config_path: str= "/root/.cache/modelscope/hub/models/LLM-Research/Fedray/FedDPA/code/lora-medical/12/", # provide only the file path, excluding the file name 'adapter_config.json'
    ...
    if not lora_weights_path.endswith(".bin"):
        ...
        if local:
            ...
            local_state = torch.load(local_model_path,map_location = lambda s,l:s)
            set_peft_model_state_dict(model,local_state,"local")
            if not auto:
                model.set_local(['local'], [weight,1-weight])
        set_peft_model_state_dict(model,lora_weights,"default")
        model.set_adapter('default')
        print(">>> default adapter lora_A norms:")
        for name, param in model.named_parameters():
            if ".lora_A.default." in name:
                print(f"{name:60s}  norm={param.norm().item():.4f}")

        # 打印 local adapter 的一些 LoRA 权重范数
        print(">>> local  adapter lora_A norms:")
        for name, param in model.named_parameters():
            if  ".lora_A.local" in name:
                print(f"{name:60s}  norm={param.norm().item():.4f}")
        print("—— End of adapter-norm check ——")
        del lora_weights
    ...
    for i,line in enumerate(lines):
        ...
        if auto:
            model.unset_local()
        tmp = {}
        if 'Question' in ques:  # 医疗数据格式
                tmp['text'] = ques['Question']
                tmp['answer'] = res
                tmp['category'] = ques.get('科室', 'unknown')  # 有些样本可能没有科室信息
        elif 'instruction' in ques:  # 原始FLAN格式
                tmp['text'] = ques['instruction']
                tmp['answer'] = res
                tmp['category'] = ques.get('category', 'unknown')
        #tmp['text'] = ques['instruction']
        #tmp['answer'] = res
